  ...
  "completion": [
    {
      ...
      "label": "函数封装模板",
      "desc": "快速生成函数定义骨架",
      "insert": "static inline int func(int a, int b) {\n    return a + b;\n}"
    }
  ]
}
```
...
**参数：**
- `language` - 语言标识（当前支持 `cpp`）
- `trigger` - 触发词（用户输入前缀时匹配）
- `label` - 补全显示文本
- `desc` - 补全描述
- `insert` - 选中后插入的文本
...
### on_editor_completion_request - 编辑器补全请求

编辑器触发补全时的预留 Hook 点位（当前用于扩展与日志，不建议阻塞执行）。

**可用变量（预留，后续会继续补充）：**
- `${current_file}` - 当前文件路径（若可用）
- `${current_dir}` - 当前目录（若可用）

```json
{
  "events": {
    "on_editor_completion_request": [
      {"node": "command", "sh": "echo '[completion] ${current_file}' >> ${plugin_path}/completion.log"}
    ]
  }
}
```
